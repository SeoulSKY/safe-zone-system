...
import { exchangeCodeAsync, makeRedirectUri, revokeAsync, TokenResponse, useAuthRequest, useAutoDiscovery } from 'expo-auth-session';
...
/**
 * @typedef {Object} UseLoginResponse an object containing nessasary infomation to implement
 * a user login
 * @property {boolean} loginReady a boolean indicating if the login function is ready/callable
 * @property {boolean} loggedIn a boolean indicating if the user is currently logged in
 * @property {function} login a function used to login the user.
 * Precondition: loginReady == true
 * Postcondition: Prompts user for login, then once authorized tokens will be no longer be null
 * @property {function} logout a function to logout the current user. This should only be
 * called when the user is currently logged in
 * Precondition: loggedIn == true
 * Postcondition: Logs out the user. Tokes will be null
 * @propery {null | TokenResponse} tokens an object of container the refresh and access token.
 * value is null until user is logged in using the login function
 */

/**
 * Handles user login for the safe-zone keycloak realm.
 * @returns {UseLoginResponse} A set of utility variables and functions to implement a user login
 * Preconditions: Keycloak server is available with the safe-zone configuration imported
