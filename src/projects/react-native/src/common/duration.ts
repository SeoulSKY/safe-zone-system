...
export class Duration {
  ...
  constructor(milliseconds: number) {
   ...
   /**
   * Converts a number to a string. If the number has less than 2 digits (n < 10),
   * then a leading zero is added when it is converted to a string.
   * 
   * @param n any positive number
   * @returns the number as a string with at least 2 digits.
   */
